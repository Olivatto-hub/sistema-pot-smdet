...
# FUNÇÃO RESTAURADA: Sistema de backup e recuperação
def criar_backup():
    ...
    timestamp = data_hora_arquivo_brasilia()
    backup_file = os.path.join(backup_dir, f"pot_smdet_backup_{timestamp}.db")
    
    try:
        # Copiar o arquivo do banco de dados
        import shutil
        shutil.copy2('pot_smdet.db', backup_file)
        return backup_file, True
    except Exception as e:
        return str(e), False

# FUNÇÃO RESTAURADA: Limpar dados históricos
def limpar_dados_historicos(conn, tipo, meses_manter=12):
    """Remove dados históricos antigos, mantendo apenas os últimos X meses"""
    try:
        data_limite = agora_brasilia() - timedelta(days=meses_manter*30)
        
        if tipo == 'pagamentos':
            conn.execute('''
                DELETE FROM pagamentos 
                WHERE data_importacao < ?
            ''', (data_limite.strftime("%d/%m/%Y às %H:%M"),))
        elif tipo == 'inscricoes':
            conn.execute('''
            ...
            conn.execute('''
                DELETE FROM metricas_mensais 
                WHERE data_calculo < ?
            ''', (data_limite.strftime("%d/%m/%Y às %H:%M"),))
        
        conn.commit()
        return True, f"Dados históricos de {tipo} limpos com sucesso"
    except Exception as e:
        return False, f"Erro ao limpar dados: {str(e)}"

# FUNÇÃO RESTAURADA: Exportar dados completos
def exportar_dados_completos(conn):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        if not df_pagamentos.empty:
            df_pagamentos.to_excel(writer, sheet_name='Pagamentos_Histórico', index=False)
        
        # Exportar inscrições
        df_inscricoes = carregar_inscricoes_db(conn)
        if not df_inscricoes.empty:
            df_inscricoes.to_excel(writer, sheet_name='Inscrições_Histórico', index=False)
        
        # Exportar métricas
        df_metricas = carregar_metricas_db(conn)
        if not df_metricas.empty:
            df_metricas.to_excel(writer, sheet_name='Métricas_Históricas', index=False)
...
# Interface principal do sistema COMPLETA
def main():
    ...
    # SEÇÃO RESTAURADA: Administração do Sistema
    with st.sidebar.expander("⚙️ Administração", expanded=False):
        ...
        if st.button("💾 Criar Backup", key="backup_btn"):
            backup_file, success = criar_backup()
            if success:
                ...
                st.error(f"❌ Erro ao criar backup: {backup_file}")
        
        # Exportar todos os dados
        if st.button("📤 Exportar Todos Dados", key="export_all"):
            with st.spinner("Exportando dados..."):
                dados_completos = exportar_dados_completos(conn)
            
            st.download_button(
                label="📥 Baixar Dados Completos",
                data=dados_completos,
                file_name=f"dados_completos_pot_{data_hora_arquivo_brasilia()}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            )
        
        # Limpeza de dados históricos
        st.markdown("---")
        st.write("**Limpeza de Dados Históricos**")
        
        tipo_limpeza = st.selectbox(
            "Tipo de dados para limpar",
            ['pagamentos', 'inscricoes', 'metricas'],
            key='tipo_limpeza'
        )
        
        meses_manter = st.slider(
            "Mantendo últimos (meses)",
            min_value=1,
            max_value=36,
            value=12,
            key='meses_manter'
        ...
        if st.button("🗑️ Limpar Dados Antigos", key="clean_old"):
            if st.warning("⚠️ Esta ação não pode ser desfeita. Tem certeza?"):
                sucesso, mensagem = limpar_dados_historicos(conn, tipo_limpeza, meses_manter)
                if sucesso:
                    st.success(mensagem)
                else:
                    st.error(mensagem)
    ...
    # Abas principais do sistema - ADICIONADA ABA DE ADMINISTRAÇÃO
    tab_principal, tab_dashboard, tab_relatorios, tab_historico, tab_estatisticas, tab_admin = st.tabs([
        "📊 Análise Mensal", 
        "📈 Dashboard Evolutivo", 
    ...
    with tab_admin:
        ...
        with col1:
            ...
            try:
                df_pagamentos_count = carregar_pagamentos_db(conn)
                df_inscricoes_count = carregar_inscricoes_db(conn)
                df_metricas_count = carregar_metricas_db(conn)
                
                st.metric("Total de Importações (Pagamentos)", len(df_pagamentos_count))
                st.metric("Total de Importações (Inscrições)", len(df_inscricoes_count))
                st.metric("Total de Métricas Salvas", len(df_metricas_count))
                
                # Tamanho do banco de dados
                if os.path.exists('pot_smdet.db'):
                    size_bytes = os.path.getsize('pot_smdet.db')
                    size_mb = size_bytes / (1024 * 1024)
                    st.metric("Tamanho do Banco de Dados", f"{size_mb:.2f} MB")
        ...
        with col2:
            ...
            if st.button("🔍 Verificar Integridade", key="check_integrity"):
                try:
                    # Testar conexões
                    test_pagamentos = carregar_pagamentos_db(conn)
                    test_inscricoes = carregar_inscricoes_db(conn)
                    test_metricas = carregar_metricas_db(conn)
                    
                    st.success("✅ Banco de dados íntegro")
                    st.info(f"- Pagamentos: {len(test_pagamentos)} registros")
                    st.info(f"- Inscrições: {len(test_inscricoes)} registros")
                    st.info(f"- Métricas: {len(test_metricas)} registros")
                    
                except Exception as e:
                    st.error(f"❌ Problema detectado: {e}")
            
            # Limpeza de cache da sessão
        ...
        # Mostrar logs recentes (simplificado)
        st.info("Logs do sistema disponíveis no console/terminal onde o Streamlit está rodando.")
