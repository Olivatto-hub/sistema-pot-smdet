...
class DatabaseManager:
    ...
    def delete_all_data(self, usuario):
        ...
        self.conn.commit()
        self.log_action(usuario, "DELETE", "Todos os dados foram excluídos.")
...
def process_file(uploaded_file):
    ...
    # Se houver coluna de mês, tenta usar, senão usa do arquivo
    if 'mes_ref' in df.columns:
         # Lógica simplificada: se a coluna existir, mantemos, mas normalizamos para numérico se possível
         pass
...
def check_login(username, password):
    # Regra: email@prefeitura.sp.gov.br e senha Smdetpot2025
    valid_domain = username.endswith("@prefeitura.sp.gov.br")
    valid_pass = password == "Smdetpot2025"
    
    if valid_domain and valid_pass:
        # Define role (Lógica simples para demo)
        if "admin" in username.lower() or "ti" in username.lower():
            return True, "admin"
        return True, "user"
    return False, None
        ...
        with st.form("login_form"):
            ...
            if submitted:
                ...
                if success:
                    ...
                    st.error("Credenciais inválidas ou domínio de e-mail incorreto.")
    ...
    if st.button("Processar e Salvar no Banco de Dados"):
        ...
        for i, file in enumerate(uploaded_files):
            ...
            if error:
                ...
                # Preview dos dados ajustados
                with st.expander(f"Visualizar dados processados: {file.name}"):
...
def admin_screen():
    ...
    if st.button("🗑️ EXCLUIR TODOS OS DADOS DO SISTEMA", type="primary"):
